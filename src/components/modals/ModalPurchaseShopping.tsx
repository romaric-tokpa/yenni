...
import { formatCFA, suggestedTransactionFeePercentFromAccount, isVaultAccountLocked } from "@/lib/constants";
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (changeLeft && changeForm.amount && Number(changeForm.amount) > 0 && changeForm.merchant.trim()) {
        ...
        showToast("Article acheté et monnaie suivie dans Prêts !");
      } else {
        showToast("Article marqué comme acheté !");
      }
      onClose();
    } catch {
  ...
  return (
    <>
        ...
        {changeLeft && (
          <div className="grid gap-3 pl-6">
            ...
            <div>
              ...
              <input className="input-field" placeholder="Ex: Épicerie du coin..." value={changeForm.merchant} onChange={(e) => setChangeForm({ ...changeForm, merchant: e.target.value })} />
            </div>
            <div>
      ...
      <div className="flex gap-3 mt-6 pt-4 border-t border-white/5">
        ...
        <button onClick={handleSubmit} disabled={loading} className="btn-primary flex-1 min-h-[44px] py-3 rounded-lg text-sm font-medium flex items-center justify-center gap-1.5">
          <Check size={16} /> {loading ? "..." : "Enregistrer"}
        </button>
      </div>
    </>
  );
