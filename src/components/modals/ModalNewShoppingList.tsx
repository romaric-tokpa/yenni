...
interface ModalNewShoppingListProps {
  onClose: () => void;
  budget: BudgetContextValue;
}

export default function ModalNewShoppingList({ onClose, budget }: ModalNewShoppingListProps) {
    ...
    <>
      <div className="flex justify-between items-center mb-5">
        <h2 className="text-base lg:text-lg font-bold flex items-center gap-2">
          <ShoppingCart size={18} className="text-amber-400" /> Nouvelle liste de courses
        </h2>
      ...
      <div className="grid gap-4">
        <div>
          ...
          <input className="input-field" placeholder="Ex: Course nourriture, Course maison" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} />
        </div>
        <div>
